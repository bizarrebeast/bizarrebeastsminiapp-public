...
import { isBetaTester, BETA_MY_WALLETS_ACTIVE, BETA_COMMUNITY_ACTIVE } from '@/lib/beta-testers';
...
const RITUAL_10_WHITELIST = [
  ...
  '0xd35dA0C9824ce664b106Ac5a526221e5fA66F433' // BB's base wallet
...
const getRitualsForWallet = (walletAddress?: string): Ritual[] => {
  ...
  // Hide rituals #10 and #11 if only MY_WALLETS beta is active (not community beta)
  ...
  if (BETA_MY_WALLETS_ACTIVE && !BETA_COMMUNITY_ACTIVE) {
    // Only show #10 and #11 to beta testers
    if (!isBetaTester(walletAddress || null)) {
      rituals = rituals.filter(r => r.id !== 10 && r.id !== 11);
      return rituals;
    }
  }
...
export default function RitualsPage() {
  ...
  const loadRitualCompletions = async (showLoading = true) => {
    ...
    try {
      ...
      // Check if ritual 10 should be included based on attestation time
      const attestationTime = localStorage.getItem('lastAttestationTime');
      if (attestationTime && wallet.address && RITUAL_10_WHITELIST.includes(wallet.address.toLowerCase())) {
  ...
  const checkForCelebration = () => {
    ...
    // Check if ALL available rituals are completed
    const allComplete = rituals.every(r => completedRituals.has(r.id));
      ...
      <div className="max-w-4xl mx-auto w-full overflow-x-hidden">
        ...
        {showCelebration && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-dark-card border-2 border-gem-gold rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
              <button
              ...
              <p className="text-xl text-white mb-6">
                You completed ALL your daily rituals!
              </p>

              <p className="text-lg text-gem-gold mb-6">
                You're a BEAST! 👹
              </p>

              <div className="mb-6 p-4 bg-dark-bg/50 rounded-lg border border-gem-crystal/20">
              ...
              <ShareButtons
                ...
                ritualData={{
                  ...
                  description: "I just completed ALL my daily BIZARRE rituals on the BizarreBeasts ($BB) Miniapp! 🔥\n\nI'm a BEAST! 👹\n\nJoin me and go BIZARRE with daily community interactions!",
                  actionUrl: "https://bbapp.bizarrebeasts.io/rituals"
                }}
                contextUrl="https://bbapp.bizarrebeasts.io/rituals"
                buttonSize="lg"
                showLabels={true}
              ...
              <button
                onClick={() => setShowCelebration(false)}
                className="mt-6 px-6 py-3 bg-gem-crystal hover:bg-gem-crystal/80 text-black font-bold rounded-lg transition-colors"
              >
                Close
              </button>
            </div>
