-- Add gallery features to contests table (SAFE VERSION)
...
-- Create a NEW view with a different name instead of dropping
CREATE OR REPLACE VIEW contest_leaderboard_v2 AS
SELECT
  s.contest_id,
  s.wallet_address,
  s.username,
...
-- Grant permissions
GRANT SELECT ON contest_leaderboard_v2 TO authenticated;
GRANT SELECT ON contest_leaderboard_v2 TO anon;

-- After verifying the new view works, you can manually:
-- 1. Update your app to use contest_leaderboard_v2
-- 2. Drop the old view: DROP VIEW contest_leaderboard;
-- 3. Rename the new view: ALTER VIEW contest_leaderboard_v2 RENAME TO contest_leaderboard;