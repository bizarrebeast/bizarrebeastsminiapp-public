...
BEGIN
  ...
  -- Check daily limit
  SELECT COALESCE(value::TEXT::BIGINT, 5000000000000000000000)
  INTO v_daily_limit
...
-- Policies for bets (users can read their own, everyone can read leaderboards)
CREATE POLICY "Users can view own bets"
  ON coin_flip_bets FOR SELECT
  USING (wallet_address = current_user);

CREATE POLICY "System can insert bets"
  ON coin_flip_bets FOR INSERT
  WITH CHECK (true);

CREATE POLICY "System can update bets"
  ON coin_flip_bets FOR UPDATE
  USING (true);
...
CREATE POLICY "Daily leaderboards are public"
  ON coin_flip_leaderboard_daily FOR SELECT
  USING (true);

CREATE POLICY "Weekly leaderboards are public"
  ON coin_flip_leaderboard_weekly FOR SELECT
  USING (true);

-- Self-exclusions (users can manage their own)
